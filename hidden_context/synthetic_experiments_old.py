    ...
    *,
    ...
    for _ in progress_bar:
        ...
        if (len(inspect.signature(reward_fn).parameters.keys()) == 2):
            identity = torch.randint(0, 3, state0.shape, device=state0.device)
...
def noisy_reward(state: torch.Tensor) -> torch.Tensor:
    state = state.squeeze(-1)
    rewards = state.clone()
    # 10% chance to flip a preference, done later
    return rewards
...
def main(  # noqa: C901
    *,
    ...
    if env_name == "1d" or env_name == "1d_identity" or env_name == "1d_cycles":
    ...
    figure.savefig(os.path.join(experiment_dir, f"{env_name}_d.png"), dpi=300)
    figure.savefig(os.path.join(experiment_dir, f"{env_name}_d.pgf"), dpi=300)
